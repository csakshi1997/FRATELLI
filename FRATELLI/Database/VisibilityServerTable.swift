...
class VisibilityServerTable: Database {
    let SQLITE_TRANSIENT = unsafeBitCast(-1, to: sqlite3_destructor_type.self)
    
    func createVisibilityServerTable() {
        let createTableQuery = """
            CREATE TABLE IF NOT EXISTS VisibilityServerTable (
        ...
        if sqlite3_exec(Database.databaseConnection, createTableQuery, nil, nil, nil) != SQLITE_OK {
            print("❌ Error creating VisibilityServerTable")
        } else {
            print("✅ VisibilityServerTable created or already exists.")
        }
    }
    
    func saveRecord(_ record: VisibilityServerModel, completion: @escaping (Bool, String?) -> Void) {
        var statement: OpaquePointer?
        let insertQuery = """
            INSERT INTO VisibilityServerTable (
    ...
    func getAllRecords() -> [VisibilityServerModel] {
        var results: [VisibilityServerModel] = []
        var statement: OpaquePointer?
        let query = "SELECT * FROM VisibilityServerTable"
        ...
        if sqlite3_prepare_v2(Database.databaseConnection, query, -1, &statement, nil) == SQLITE_OK {
            while sqlite3_step(statement) == SQLITE_ROW {
                ...
                let record = VisibilityServerModel(
                    localId: localId,
                    userId: userId,
                    fileType: fileType,
                    fileName: fileName,
                    ...
                    dealerDistributorCorpId: dealerDistributorCorpId,
    ...
    func getUnsyncedRecords() -> [VisibilityServerModel] {
        var results: [VisibilityServerModel] = []
        var statement: OpaquePointer?
        let query = "SELECT * FROM VisibilityServerTable WHERE isSync = '0'"
        ...
        if sqlite3_prepare_v2(Database.databaseConnection, query, -1, &statement, nil) == SQLITE_OK {
            ...
            print("❌ Failed to fetch unsynced VisibilityServerTable records.")
        }
    ...
    func updateSyncStatus(forLocalId localId: Int) {
        var statement: OpaquePointer?
        let query = "UPDATE VisibilityServerTable SET isSync = '1' WHERE localId = ?"
    ...
    func updatePublicURL(forLocalId localId: Int, url: String) {
        var statement: OpaquePointer?
        let updateQuery = "UPDATE VisibilityServerTable SET imagePublicUrl = ? WHERE localId = ?"
